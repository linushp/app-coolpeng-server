...
//		System.out.println();
    ...
    private Object toJSONColumnObject(Field field, ResultSet rs) {
        ...
        try {
            String fieldName = com.coolpeng.framework.utils.StringUtils.camelToUnderline(field.getName());
            valueText = rs.getString(fieldName);
    ...
    private Object toRelationFieldObject(Field entityField, ResultSet rs) throws InstantiationException, IllegalAccessException {
        ...
        //不处理JSON字段
        if (ReflectUtils.isJSONColumn(entityField)) {
            return null;
        }
